...
def juego_ahorcado():
    ...
    print(mostrar_progreso(palabra_secreta, letras_adivinadas), "La cantidad de letras de la palabras es:", len(palabra_secreta))
    ...
    while not juego_terminado and intentos > 0:
        ...
        if len(adivinanza) != 1 or not adivinanza.isalpha():
            ...
            print("Ya has utilizado esa letra, por favor prueba con otra")
        else:
            letras_adivinadas.append(adivinanza)
            if adivinanza in palabra_secreta:
                print(
                    f"¡Muy bien! Has acertado, la letra '{adivinanza}' está presente en la palabra secreta"
                )
            else:
                intentos -= 1
                print(f"La letra' {adivinanza} 'no esta presente en la palabra secreta")
    ...
    # Si pierde porque se queda sin intentos
    if intentos == 0:
        palabra_secreta = palabra_secreta.capitalize()
        print(
            f"Lo sentimos mucho, se te han acabado los intentos. La palabra secerta era {palabra_secreta}"
        )
